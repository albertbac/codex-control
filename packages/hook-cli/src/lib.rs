...
use codex_core::{
    evaluate_policy, normalize_hook_payload, parse_hook_stdin, redact_event_payload,
    sanitize_public_output, LocalStore, PolicyVerdict, PERMISSION_REQUEST_DENY_RESPONSE_JSON,
    PRE_TOOL_USE_DENY_RESPONSE_JSON,
};
use serde_json::json;
...
fn policy_impl(input: &str) -> Result<String> {
    ...
    let output = match evaluate_policy(&event) {
        ...
        PolicyVerdict::DenyPreToolUse(_) => PRE_TOOL_USE_DENY_RESPONSE_JSON.to_string(),
        PolicyVerdict::DenyPermissionRequest(_) => {
            PERMISSION_REQUEST_DENY_RESPONSE_JSON.to_string()
        }
    };
    Ok(output)
}
...
fn doctor_impl() -> Result<String> {
    ...
    let _config: toml::Value =
        toml::from_str(EXAMPLE_CONFIG).context("embedded config.toml is invalid")?;
