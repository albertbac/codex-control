...
static SESSION_ASSIGNMENT: Lazy<Regex> = Lazy::new(|| {
  Regex::new(r"(?i)\b(session(?:[_-]?token)?|access[_-]?token|refresh[_-]?token|cookie)\b\s*[:=]\s*([^\s,;]+)")
...
pub fn sanitize_public_output(input: &str) -> String {
  ...
  PRIVATE_PATH.replace_all(&redacted, "[REDACTED_PATH]").to_string()
}
...
mod tests {
  ...
  fn redacts_cookie_and_session_tokens() {
    let redacted = redact_text("Cookie: session=abcdef1234567890 session_token=abcdef1234567890");
  ...
  fn redacts_private_paths_in_public_output() {
    let sanitized = sanitize_public_output("unable to read transcript at /Users/example/private/transcript.jsonl");
