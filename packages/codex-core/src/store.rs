...
impl LocalStore {
  ...
  fn list_events_from_spool(&self, session_id: &str) -> Result<Vec<CodexEvent>> {
    ...
    for line in contents.lines() {
      ...
      let event = serde_json::from_str::<CodexEvent>(line).context("invalid event in spool file")?;
...
fn tool_response_failed(payload: &Value) -> bool {
  ...
  if tool_response.get("exit_code").and_then(Value::as_i64).unwrap_or(0) != 0 {
    return true;
  }
...
mod tests {
  ...
  fn stale_sessions_become_unknown_when_process_disappears() {
    ...
    assert!(matches!(reconciled, SessionStatus::Unknown | SessionStatus::Finished));
  }
