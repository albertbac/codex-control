...
pub fn dashboard_snapshot() -> anyhow::Result<Vec<DashboardSession>> {
    ...
    for mut session in sessions {
        ...
        let is_stale = matches!(
            status,
            codex_core::SessionStatus::Unknown | codex_core::SessionStatus::Finished
        );
        let approval_state = approval_state_label(&status);

        session.status = status;
        session.last_command = session.last_command.or_else(|| {
            transcript
                .as_ref()
                .and_then(|value| value.last_command.clone())
        });
        session.last_assistant_message = session.last_assistant_message.or_else(|| {
            transcript
                .as_ref()
                .and_then(|value| value.last_assistant_message.clone())
        });
        ...
        dashboard.push(DashboardSession {
            ...
            process,
            source: if store.mode().as_str() == "sqlite" {
                "sqlite".to_string()
            } else {
                "spool".to_string()
            },
    ...
    dashboard.sort_by(|left, right| right.session.updated_at.cmp(&left.session.updated_at));
    Ok(dashboard)
}
...
pub fn settings_info() -> anyhow::Result<SettingsInfo> {
    ...
    Ok(SettingsInfo {
        ...
        notes: vec![
            ...
            "Hooks are guardrails and session telemetry, not a universal interception layer."
                .to_string(),
