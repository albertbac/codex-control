...
use crate::session_store::{
    dashboard_snapshot as build_dashboard, git_diff_preview, session_timeline as build_timeline,
    settings_info as build_settings,
};
...
pub fn terminate_process(pid: i64, confirm: bool) -> Result<(), String> {
    if !confirm {
        return Err(
            "explicit confirmation is required before terminating a local Codex process"
                .to_string(),
        );
    }
    Command::new("kill")
